...
//                    'pourcentage' => $pourcentage,
    ...
    public function calculDemandeAbsenceAction(Request $request) {
        ...
        if ($form->isValid()) {
            ...
            //Addition du nombre de jours total et de la date de debut
            $tms_add = $tms_nbjTotal + $tms_dateDebut;
//            $tms_add_nbjF = $tms_add - $nbSecondes;
...
//            $datetFinale = date('Y/m/d H:i:s', $tms_add_nbjF);
            ...
            if ($jourDateRetour == 0) {
                ...
                //On rajoute le nombre de dimanches total au nombre de jours totale pour Obtenir la date de retour
                $nombreJours += $joursSup3;


                //Conversion du nombre de jours séparant la date de fin de la date de retour en timestamp
                $tms_nbjf = $nombreJours * $nbSecondes;
...
//            var_dump("verif (date fin)");
//            var_dump($nombreJoursFin);
//            var_dump($nb_jr_ferie);
//            die();
...
//            die();
    ...
    public function calculDemandeCongeForSalarieAction(Request $request) {
        ...
        if ($form->isValid()) {
            ...
            //On rajoute le nombre de dimanches total au nombre de jours totale pour Obtenir la date de retour
            $nombreJoursTotal += $joursSup2 - $joursSup1;
//        $nombreJoursTotal = $nombreJoursTotal - $joursSup1 ;
            $nbjf = $nombreJoursTotal;
...
//           $unite = $salarie->getUnite();
//           var_dump($demande->getPieceJointe());
//           die();
            //Calcul des données de fin et retour.
            // Recuperation du contenu des champs nombre de jours et date de debut
            $nombreJours = $demande->getNbJoursOuvrables() - 1;
...
//           $form = $this->createCongeBySupForm($demande);   
    ...
    public function createDemandePermissionBySupAction(Request $request) {
        ...
        if ($form->isValid()) {
            ...
            if ($demande->getMotif() == "Fêtes musulmanes") {
...
//                    }
    ...
    public function createDemandeAbsenceAction(Request $request) {
        ...
        if ($form->isValid()) {
            ...
            if ($demande->getMotif() == "Fêtes musulmanes" || $demande->getMotif() =="FÃªtes musulmanes") {
                $demande->setTypeDemande('Absence exceptionnelle');
            }
...
//    ############################## FONCTIONS ADDITIONNELLES #############################
    ...
    public function mettreAjourCompteurs($demande, $salarie, $notification) {
        ...
        if ($demande->getTypeDemande() == "Conge") {
            ...
            $notification->setSuperieurN1($valideurConnecte);
            $em->persist($notification);
            $em->persist($demande);
            $em->persist($conge);
            $em->persist($droits);
        ...
        if ($demande->getTypeDemande() == "Permission") {
            ...
            if (($droits->getSoldePermissions() - $demande->getNbJoursOuvrables()) >= 0) {
                //Droits
                $droits->setPermissionsPrises($demande->getNbJoursOuvrables());
            ...
            if (($droits->getSoldePermissions()) - ($demande->getNbJoursOuvrables()) < 0 and ( $droits->getSoldePermissions()) >= 0) {
                if (($droits->getSoldePermissions()) == 0) {
                    ...
                    $historiqueDroits->setSoldeCongeNouveau(($droits->getTotalDroitsAprendre()) - ($demande->getNbJoursOuvrables()));
                ...
                if (($droits->getSoldePermissions()) > 0) {
                    ...
                    $delta = $demande->getNbJoursOuvrables() - $resultat;
    ...
    public function validationNiveau1($demande, $valideurConnecte, $notification) {
        ...
        $notification->setMessageDemandeur("Demande de " . $demande->getTypeDemande() . " mise à jour (1e validation) par " . $valideurConnecte->getPoste() . ".");
        $notification->setMessageValideurEnCours("Vous avez validé la demande de " . $demande->getTypeDemande() . " de " . $demande->getSalarie() . ".");
        $notification->setMessageValideurSuivant("Demande de " . $demande->getTypeDemande() . " de " . $demande->getSalarie()->getCivilite() . " " . $demande->getSalarie()->getNomprenom() . " à valider.");
        $notification->setDemande($demande);
    }
    ...
    public function validationNiveau2($demande, $valideurConnecte, $notification) {
        ...
        $notification->setMessageDemandeur("Demande de " . $demande->getTypeDemande() . " mise à jour (2e validation) par " . $valideurConnecte->getPoste() . ".");
        $notification->setMessageValideurEnCours("Vous avez validé la demande de " . $demande->getTypeDemande() . " de " . $demande->getSalarie() . ".");
        $notification->setMessageValideurSuivant("Demande de " . $demande->getTypeDemande() . " n°" . $demande->getId() . " de " . $demande->getSalarie()->getCivilite() . " " . $demande->getSalarie()->getNomprenom() . " à valider.");
        $notification->setDemande($demande);
    }
